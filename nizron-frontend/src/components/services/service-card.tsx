...
export default function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative h-full bg-white/[0.03] backdrop-blur-xl rounded-3xl border border-white/10 p-7 flex flex-col transition-all duration-300 hover:border-white/20 hover:bg-white/[0.05]"
    >
      <div className="flex-1 space-y-5 relative z-10">
        {/* Category Badge + Title */}
        <div className="space-y-2">
          <div className="inline-flex px-2 py-0.5 bg-white/[0.05] border border-white/10 text-[10px] font-bold uppercase tracking-wider text-slate-500 rounded-md">
            {category}
          </div>
          ...
          <div className="pt-1">
            ...
            <div className="flex flex-wrap gap-1.5">
              {service.technologies.slice(0, 4).map((tech, idx) => (
                <span 
                  key={tech.id || idx} 
                  className="px-2.5 py-1 bg-white/[0.04] border border-white/[0.07] rounded-lg text-[10px] font-medium text-slate-400"
                >
                  {tech.name}
                </span>
              ))}
              {service.technologies.length > 4 && (
                <span className="px-2.5 py-1 bg-white/[0.02] border border-white/5 rounded-lg text-[10px] text-slate-600">
                  +{service.technologies.length - 4}
                </span>
              )}
            </div>
